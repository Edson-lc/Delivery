  ...
  const processOrder = async () => {
    ...
    const attemptOrder = async () => {
      ...
      if (selectedPaymentMethod?.tipo === 'dinheiro') {
        ...
        orderData.bandeiraCartao = selectedPaymentMethod.bandeira;
        orderData.finalCartao = selectedPaymentMethod.final_cartao;
        orderData.nomeTitular = selectedPaymentMethod.nome_titular;
      } else if (selectedPaymentMethod?.tipo === 'stripe_new' || selectedPaymentMethod?.tipo === 'stripe_saved') {
        // Para pagamentos Stripe, o processamento será feito após a criação do pedido
        orderData.metodoPagamento = 'stripe';
        orderData.status = 'pendente_pagamento'; // Aguardando confirmação do Stripe
        
        if (selectedPaymentMethod?.tipo === 'stripe_saved') {
          orderData.stripePaymentMethodId = selectedPaymentMethod.paymentMethodId;
          orderData.bandeiraCartao = selectedPaymentMethod.card?.brand;
          orderData.finalCartao = selectedPaymentMethod.card?.last4;
        }
      }
