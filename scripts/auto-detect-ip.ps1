...
if (-not $ip) {
    ...
    Write-Host "   2. Use um serviço de DNS dinâmico" -ForegroundColor White
...
$backendEnvFile = "server/.env"
if (Test-Path $backendEnvFile) {
    $backendContent = Get-Content $backendEnvFile -Raw
    if ($backendContent -notlike "*$ip*") {
        # Atualizar CORS_ORIGIN no backend
        $backendContent = $backendContent -replace "CORS_ORIGIN=.*", "CORS_ORIGIN=http://$ip`:5173,http://localhost:5173"
        $backendContent | Out-File -FilePath $backendEnvFile -Encoding UTF8
        Write-Host "✅ Configuração do backend atualizada" -ForegroundColor Green
    }
}
