...
export default function RestaurantMenuPage() {
  ...
  const handleUpdateQuantity = (index, newQuantity) => {
    ...
    updatedCart.itens[index].quantidade = newQuantity;
    
    // Recalcular subtotal
    updatedCart.subtotal = updatedCart.itens.reduce((total, item) => {
                ...
                {Object.entries(groupedItems).map(([category, items]) => (
                  <section key={category} id={`category-${category}`}>
                    ...
                    <div className="divide-y border-t border-b">
                      {items.map(item => (
                        <MenuItemCard key={item.id} item={item} onAddToCart={addToCart} cart={cart} onCartClick={handleCartClick} />
                      ))}
                    </div>
                  </section>
                ))}
