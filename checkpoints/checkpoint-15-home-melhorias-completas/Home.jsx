...
const SearchWithSuggestions = lazy(() => import("../components/public/SearchWithSuggestions"));
            ...
            <div className="max-w-2xl mx-auto px-4">
              <Suspense fallback={
                <div className="flex items-center bg-white rounded-2xl shadow-lg border border-gray-200 p-2">
                  ...
                  <div className="ml-2 w-20 h-12 bg-orange-200 rounded-xl animate-pulse"></div>
                </div>
              }>
                <SearchWithSuggestions
                  value={activeFilters.search}
                  onChange={(value) => setActiveFilters(prev => ({ ...prev, search: value }))}
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-4">
                {['Pizza', 'Hambúrguer', 'Japonesa', 'Brasileira'].map((category) => (
                  <button
                    ...
                    className="px-3 py-2 bg-white/20 backdrop-blur-sm rounded-full text-xs sm:text-sm font-medium hover:bg-white/30 transition-colors"
                  >
                    {category}
                  </button>
                ))}
