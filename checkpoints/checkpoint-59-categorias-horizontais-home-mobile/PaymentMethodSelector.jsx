...
export default function PaymentMethodSelector({ 
  ...
  const formatCurrencyValue = (value) => {
    ...
    // Se tem apenas números, adicionar vírgula para centavos
    if (parts.length === 1 && parts[0].length > 2) {
      const integerPart = parts[0].slice(0, -2);
      const decimalPart = parts[0].slice(-2);
            ...
            <div className="space-y-2">
              ...
              {selectedPaymentMethod?.tipo === 'dinheiro' && (
                <div className="space-y-2">
                  ...
                  {cashAmount && parseCurrencyValue(cashAmount) < totalAmount && parseCurrencyValue(cashAmount) > 0 && (
                    <div className="text-sm text-red-600 bg-red-50 p-2 rounded-md">
                      Valor insuficiente. Mínimo: €{totalAmount.toFixed(2)}
                    </div>
                  )}
                  
                  {/* Mostrar confirmação apenas se valor for válido */}
                  {(!cashAmount || cashAmount.trim() === '' || parseCurrencyValue(cashAmount) >= totalAmount) && (
